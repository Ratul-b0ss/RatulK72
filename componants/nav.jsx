...
const Nav = ({value}) => {
  ...
  const menubtnhoverref = useRef(null)
  

  const [isMenuon, setisMenuon] = value;
  return (
    <div className='fixed top-[0%] z-30 w-full flex items-start justify-between'>
      <div className='pl-2.5 pt-2.5 min-w-fit'>
        <div className="logo w-[8.6vw] "><svg className='w-full h-full'  xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 103 44" >
      ...
      <div
        onMouseEnter={() => { menubtnhoverref.current.style.height = "100%" }}
        onMouseLeave={() => { menubtnhoverref.current.style.height = "0%" }}
        onClick={() =>{setisMenuon(true)}} 
        className="menubtn bg-black w-52 h-12 relative overflow-hidden cursor-pointer pointer-events-auto"
      >
        <div
          ref={menubtnhoverref}
          className="absolute top-0 left-0 w-full h-0 bg-[#D3FD50] transition-all duration-300 ease-in-out z-10"
        />
        <div className="relative z-20 flex flex-col gap-[0.3vw] justify-center items-end pr-[2.3vw] w-full h-full">
          
          <div className="w-12 h-0.5 bg-white group-hover:bg-black transition-colors duration-300"></div>
          <div className="w-6 h-0.5 bg-white group-hover:bg-black transition-colors duration-300"></div>
        </div>
      </div>
    </div>
